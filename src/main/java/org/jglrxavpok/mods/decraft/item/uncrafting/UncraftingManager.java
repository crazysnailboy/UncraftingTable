...
public class UncraftingManager 
{
	 ...
	 */
	public static List<NonNullList<ItemStack>> findMatchingRecipes(ItemStack itemStack)
	{
		...
		{
			ItemStack recipeOutput = recipe.getRecipeOutput();
			if (ItemStack.areItemsEqualIgnoreDurability(itemStack, recipeOutput)) // && recipeOutput.getCount() <= itemStack.getCount())
			{
				RecipeHandler handler = getRecipeHandler(recipe);
				if (handler != null)
				{
	...
	public static UncraftingResult getUncraftingResult(EntityPlayer player, ItemStack itemStack)
	{
		...
		if (uncraftingResult.minStackSizes.size() > 0 && itemStack.getCount() < uncraftingResult.getMinStackSize())
		{
...
//		// *** copied from com.jaquadro.minecraft.storagedrawers.config.OreDictRegistry ***
	...
	public static NonNullList<ItemStack> removeItemsFromOutputByDamage(ItemStack stack, NonNullList<ItemStack> craftingGrid)
	{
